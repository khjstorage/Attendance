...
public class AttendanceDBBean {
	...
	public void lookupAtt(String pId, List<AttendanceDataBean> pList) {
		...
		/*
		 * select a.ID, b.ATT_DT, b.ATT_YN, b.EARLY_YN, b.ATT_TM, b.EXIT_TM from
		 * student a inner join attendance b on a.id = b.id where a.id=binary ?
		 */
	...
	public String[] monthMylist(String pId, String pDate) {
		...
		/*
		 * SELECT A.NAME, A.ID, B.ATT_CNT, B.EARLY_CNT FROM STUDENT A RIGHT
		 * JOIN(SELECT SUM(CASE WHEN A.ATT_YN = 'Y' THEN 1 ELSE 0 END)ATT_CNT,
		 * SUM(CASE WHEN A.EARLY_YN = 'Y' THEN 1 ELSE 0 END)EARLY_CNT FROM
		 * ATTENDANCE A WHERE A.ATT_DT >= STR_TO_DATE('2016-09-01','%Y-%m-%d')
		 * AND A.ATT_DT <= STR_TO_DATE('2016-09-31','%Y-%m-%d') AND A.ID = '3')B
		 * ON ID='3';
		 */
	...
	public void monthAllList(String pDate,  List<MonthlyDataBean> pList) {
		...
		/*
		 * SELECT A.NAME, A.ID, B.ATT_CNT, B.EARLY_CNT FROM STUDENT A LEFT JOIN
		 * ( SELECT A.ID, SUM(CASE WHEN A.ATT_YN = 'Y' THEN 1 ELSE 0 END)
		 * ATT_CNT, SUM(CASE WHEN A.EARLY_YN = 'Y' THEN 1 ELSE 0 END) EARLY_CNT
		 * FROM ATTENDANCE A WHERE A.ATT_DT >=
		 * STR_TO_DATE('2016-09-01','%Y-%m-%d') AND A.ATT_DT <=
		 * STR_TO_DATE('2016-09-31','%Y-%m-%d') GROUP BY ID ) B ON A.ID = B.ID
		 */
	...
	public void attCheck(String pId) {
		...
		/*
		 * UPDATE student a INNER JOIN attendance b on a.ID = b.ID SET b.ATT_YN
		 * ='Y', b.ATT_TM = CURRENT_TIMESTAMP WHERE a.ID=binary 2 AND b.ATT_DT =
		 * 3(today)
		 */
	...
	public void exitCheck(String pId) {
		...
		/*
		 * UPDATE student a INNER JOIN attendance b on a.ID = b.ID SET b.ATT_YN
		 * ='Y', b.EXIT_TM = CURRENT_TIMESTAMP WHERE a.ID=binary 2 AND b.ATT_DT
		 * = 3(today)
		 */
